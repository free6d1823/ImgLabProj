...
{
	if(m_eCurrentProcessMode != PM_LDC) {
		if (m_pProcessTool) {
			delete m_pProcessTool;
			m_pProcessTool = NULL;
		}
	}
	m_eCurrentProcessMode = PM_LDC;

	if(!m_pProcessTool) {
		m_pProcessTool = (ProcessTool*) new LdcView();
		m_pProcessTool->Create(IDD_LDC, this);
		m_pProcessTool->SetImgFile(GetImage());
	}
...
void WorkView::OnUpdateProcessLdc(CCmdUI *pCmdUI)
{
	if ( GetImage()){
		pCmdUI->Enable(TRUE);
		pCmdUI->SetRadio(m_eCurrentProcessMode == PM_LDC);
	} else {
		pCmdUI->Enable(FALSE);
	}
}
...
{
	if(m_eCurrentProcessMode != PM_FEC ) {
		if (m_pProcessTool) {
			delete m_pProcessTool;
			m_pProcessTool = NULL;
		} 
	...
	m_eCurrentProcessMode = PM_FEC;

	if(!m_pProcessTool) {
		m_pProcessTool =  (ProcessTool*) new FecTool();
		m_pProcessTool->Create(IDD_FEC, this);
		m_pProcessTool->SetImgFile(GetImage());
		//create 
	}
	if(!m_pToolGride) {
...
void WorkView::OnUpdateProcessFec(CCmdUI *pCmdUI)
{
	if ( GetImage()){
		pCmdUI->Enable(TRUE);
		pCmdUI->SetRadio(m_eCurrentProcessMode == PM_FEC);
	} else {
		pCmdUI->Enable(FALSE);
	}
}
